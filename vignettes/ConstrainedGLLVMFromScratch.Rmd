...
Now we create the covariate data. We only need this for the first data column and the LV column (the last colunm). We add an index, `X.eps`, for the $\epsilon$'s:
...
At the moment is isn't working well. The column effects should all be about 1, so the first column is too large. The covaraites get teh sign right, but the values are too large. We can look at the column effects, which ar OK except that (with the exception of column 1) they are too small.

```{r ColEffs, fig.height=4, fig.width=5}
...
So it looks like this might be partly working, but there is some scaling issue messing things up.
