...
```{r CreateBiggerData}
  ...
  LV.true <- X%*%CovBeta+mvtnorm::rmvnorm(NRows,rep(0,nLVs),diag(nLVs))
...
dat2 <- rbind(dat, RowNAs)
dat3 <- cbind(LV = c(rep(NA, nrow(dat)), rep(0, NRows)),dat2)
...
#not bad
vegan::procrustes(c(ColEffs),ColScores[,1],symmetric=T)
```
