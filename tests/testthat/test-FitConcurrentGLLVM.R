...
test_that("FitConcurrentGLLVM works correctly", {
  #  skip_on_cran()
  set.seed(2021)
  ...
  model.X <- FitConcurrentGLLVM(Y=Y.mat, X=X, nLVs=2, Family="poisson",
                                 control.inla=list(control.vb=list(emergency=30),int.strategy="eb"))
  model.X1 <- FitConcurrentGLLVM(Y=Y.mat, X=X, nLVs=1, Family="gaussian",
                                  control.inla=list(control.vb=list(emergency=30),int.strategy="eb"))#just here because it should be tested as a special case

  # Test errors
  expect_error(FitConcurrentGLLVM(Y=Y.mat, X=X, nLVs=-1),
               "nLVs should be positive")
  expect_error(FitConcurrentGLLVM(Y=Y.mat, X=X, nLVs=26),
               "Must have fewer LVs than columns: reduce nLVs")
  expect_error(FitConcurrentGLLVM(Y=1:5, X=X, nLVs=1),
               "Y should be a matrix or data frame")
  expect_error(FitConcurrentGLLVM(Y=Y.mat, X=data.frame(x1=1:27), nLVs=1),
