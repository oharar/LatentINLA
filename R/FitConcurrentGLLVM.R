...
FitConcurrentGLLVM <- function(Y, X, formula = NULL, nLVs=1, Family="gaussian",
...
#    X <- model.matrix(formula, data.frame(X))
  ...
  # Need to add .lv1 etc. to formula passed to function.
  #  Not sure how to do random part...
  ...
  if(is.null(formula)) {
    ...
    f.tmp <- update(formula("Y ~ 1"), formula)
  }
  ...
  #  model <- INLA::inla(Formula, data=Data, family = Fam,   control.inla = list(int.strategy = "eb"))
